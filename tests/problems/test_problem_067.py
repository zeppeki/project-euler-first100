#!/usr/bin/env python3
"""
Tests for Problem 067: Maximum Path Sum II
"""
...
def load_problem_module():  # type: ignore
    """動的にproblem_067モジュールをロード"""
    problem_path = Path(__file__).parent.parent.parent / "problems" / "problem_067.py"
    spec = importlib.util.spec_from_file_location("problem_067", problem_path)
    if spec is None:
        raise ImportError("Could not load problem module")
    module = importlib.util.module_from_spec(spec)
    sys.modules["problem_067"] = module
...
class TestHelperFunctions:
    ...
    def test_get_problem_triangle(self, problem: Any) -> None:
        ...
        assert len(result) == 100  # 100行
        assert len(result[0]) == 1  # 最上行は1要素
        assert len(result[-1]) == 100  # 最下行は100要素
        assert result[0][0] == 59  # 最上位の値
...
class TestSolutionCorrectness:
    ...
    def test_small_triangles(self, problem: Any, triangle: Any, expected: Any) -> None:
        """小さな三角形での全解法テスト"""
        # solve_naiveは大きな三角形では実用的でないため小さなもののみ
        if len(triangle) <= 5:
...
class TestSolutionConsistency:
    ...
    def test_optimized_and_mathematical_agree_problem(self, problem: Any) -> None:
        """本問題で最適化解法と数学的解法が一致することを確認"""
        triangle = problem.get_problem_triangle()

        # solve_naiveは100行では実用的でないため除外
        optimized_result = problem.solve_optimized(triangle)
        math_result = problem.solve_mathematical(triangle)
...
class TestPerformance:
    ...
    def test_naive_solution_warning(self, problem: Any) -> None:
        """solve_naiveが大きな三角形では実用的でないことを確認"""
        # 中程度のサイズでも警告として実行時間をテスト
        medium_triangle = [[1] * (i + 1) for i in range(10)]  # 10行の三角形

        import time

        start = time.time()
        problem.solve_naive(medium_triangle)
        naive_time = time.time() - start

        # 最適化解法と比較
        start = time.time()
        problem.solve_optimized(medium_triangle)
        optimized_time = time.time() - start

        # solve_naiveは指数時間のため、より多くの時間がかかることを確認
        # ただし、10行程度であれば実行可能
        assert naive_time >= optimized_time
...
class TestDataFileIntegration:
    ...
    def test_data_file_exists(self) -> None:
        """三角形データファイルが存在することを確認"""
        from pathlib import Path

        data_file = Path(__file__).parent.parent.parent / "data" / "0067_triangle.txt"
        assert data_file.exists()

    def test_data_file_format(self, problem: Any) -> None:
        """データファイルの形式が正しいことを確認"""
        triangle = problem.get_problem_triangle()
        ...
        for i, row in enumerate(triangle):
            assert len(row) == i + 1  # i行目は(i+1)個の要素
