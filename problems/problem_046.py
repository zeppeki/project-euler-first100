...
def test_solutions() -> None:
    ...
    for limit, expected in test_cases:
        assert solve_naive(limit) == expected, (
            f"Naive solution failed for limit {limit}"
        )
        assert solve_optimized(limit) == expected, (
            f"Optimized solution failed for limit {limit}"
        )
        assert solve_mathematical(limit) == expected, (
            f"Mathematical solution failed for limit {limit}"
        )

    # すべての解法が同じ結果を返すことを確認
    result_naive = solve_naive(10000)
...
def main() -> None:
    """メイン関数"""
    print("Project Euler Problem 046: Goldbach's other conjecture")
    print("=" * 60)
    ...
    print(f"素直な解法: {result_naive} (実行時間: {naive_time:.4f}秒)")

    # 最適化解法
    start_time = time.time()
    result_optimized = solve_optimized(10000)
    optimized_time = time.time() - start_time
    print(f"最適化解法: {result_optimized} (実行時間: {optimized_time:.4f}秒)")

    # 数学的解法
    start_time = time.time()
    result_mathematical = solve_mathematical(10000)
    mathematical_time = time.time() - start_time
    print(f"数学的解法: {result_mathematical} (実行時間: {mathematical_time:.4f}秒)")
    ...
    for num in examples:
        if num % 2 == 1 and not is_prime(num):
            ...
            print(f"{num}: {'成り立つ' if can_write else '成り立たない'}")
            if can_write:
                # 具体的な分解例を表示
                for prime in primes:
                    if prime >= num:
