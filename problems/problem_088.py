...
def test_solutions() -> None:
    ...
    # 問題文の例: k=2〜6の場合
    test_k = 6
    expected = 30  # 4 + 6 + 8 + 12 = 30 (8は1回のみカウント)

    result_naive = solve_naive(test_k)
    result_optimized = solve_optimized(test_k)
    result_mathematical = solve_mathematical(test_k)

    print(f"Test (k=2 to {test_k}):")
    ...
    result_naive2 = solve_naive(test_k2)
    result_optimized2 = solve_optimized(test_k2)
    result_mathematical2 = solve_mathematical(test_k2)

    print(f"\nTest (k=2 to {test_k2}):")
    print(f"  Naive: {result_naive2} (expected: {expected2})")
    print(f"  Optimized: {result_optimized2} (expected: {expected2})")
    print(f"  Mathematical: {result_mathematical2} (expected: {expected2})")

    assert result_naive2 == expected2
    assert result_optimized2 == expected2
    assert result_mathematical2 == expected2
...
def main() -> None:
    ...
    # 各解法で計算
    start = time.time()
    result1 = solve_naive(max_k)
    time1 = time.time() - start

    start = time.time()
    result2 = solve_optimized(max_k)
    time2 = time.time() - start

    start = time.time()
    result3 = solve_mathematical(max_k)
    time3 = time.time() - start

    print(f"\nProblem 88: Sum of minimal product-sum numbers for 2≤k≤{max_k}")
